...
- (void)putCollection:(NSArray *)collection {
    ...
    for (int i = 0 ; i < collection.count; i++) {
        [self putLongLE:[EosUtils string_to_long: collection[i]]];
    }
}
...
+ (NSData *)getIdData {
    ...
    NSData *Id = [NSData dataWithBytes:byte length:32];
    return Id;
}
...
+ (NSData *)getBytesForSignature:(NSData *)chainId andParams:(NSDictionary *)paramsDic andCapacity:(int)capacity{
    ...
    [writer putLongLE:[EosUtils string_to_long:actionDic[@"account"]]];
    [writer putLongLE:[EosUtils string_to_long:actionDic[@"name"]]];
    ...
    [writer putLongLE:[EosUtils string_to_long:authorizationDic[@"actor"]]];
    [writer putLongLE:[EosUtils string_to_long:authorizationDic[@"permission"]]];
    
    NSString *data = [actionDic objectForKey:@"data"];
    if (NULL != data) {
        NSData *dataAsBytes = [EosUtils convertHexStrToData:data];
    ...
    [writer putBytes:[self getIdData]];
